...
class serverinfo extends Command {
    constructor() {
        super({
            name: 'serverinfo',
            category: 'utils',
            description: 'Cette commande donne des informations sur le serveur !',
            usage: 'serverinfo',
            example: ['serverinfo', 'si'],
            aliases: ['si']
        });
    }
    /*
    ...
    const today = {
     ...
     await message.channel.send({ 
            embed: { 
                color: 0x2f3136, 
                author: { 
                    name: `Statistiques du serveur : ${message.guild.name}`, 
                    icon_url: message.guild.iconURL({ display: true }) }, 
                    timestamp: new Date(), 
                    description: `:id: \`Identifiant\` : **${message.guild.id}**\n:bust_in_silhouette: \`Serveur créé le\` : **${moment(message.guild.createdAt).format('DD/MM/YYYY à HH:mm:ss')}**\n:bar_chart: \`Statistiques du\` : **${moment(ui[0].date).format('DD/MM/YYYY à HH:mm:ss')}**\n:bar_chart: \`Statistiques comptées depuis le\` : **${moment(message.guild.members.cache.get(client.user.id).joinedAt).format('DD/MM/YYYY à HH:mm:ss')}**\n\u200b`, 
                    fields: [
